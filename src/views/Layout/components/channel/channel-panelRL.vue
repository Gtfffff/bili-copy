...
let length = ref(
  computed(() => {
    return home.channelCategory.length
  })
)
</script>
...
<template>
  <div
    class="grid grid-cols-12 grid-rows-2 grid-flow-row gap-[10px] border-r-[1px] border-solid border-gray-400 pr-[30px] w-[1135px] flex-auto"
  >
    <template v-if="length < 25">
      <a
        v-for="(item, i) in home.channelCategory"
        :key="i"
        href="#"
        class="box-content inline-block w-full border-solid border-gray-400 rounded-md bg-slate-200 text-center font-normal text-[14px] leading-[30px] h-[30px]"
      >
        {{ item }}
      </a>
    </template>
    <template v-else>
      <!-- 范围内 -->
      <a
        v-for="(item, i) in home.channelCategory.slice(0, 23)"
        :key="i"
        href="#"
        class="box-content inline-block w-full border-solid border-gray-400 rounded-md bg-slate-200 text-center font-normal text-[14px] leading-[30px] h-[30px] hover:bg-slate-400"
      >
        {{ item }}
      </a>
      <!-- 更多按钮 -->
      <div class="relative group">
        ...
        >
          ...
          <svg
            ...
            class="transition duration-300 inline-block ml-1 group-hover:rotate-180"
          >
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
        ...
        <!-- 超出部分 -->
        <div class="hidden group-hover:block absolute pt-2 left-full -translate-x-full z-[2]">
          <div
            class="flex flex-wrap justify-between w-[150px] rounded-lg bg-slate-50 px-2 border-gray-200 border-solid border-[1px]"
          >
            <div
              v-for="(item, i) in home.channelCategory.slice(23)"
              :key="i"
              class="rounded-md text-center font-normal text-[14px] leading-[30px] h-[30px] px-2 hover:bg-slate-400 my-[1px]"
            >
              <a href="#"></a>
              <span>{{ item }}</span>
            </div>
