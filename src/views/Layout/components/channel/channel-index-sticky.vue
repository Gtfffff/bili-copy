...
const { home } = useStore()
const arr = ['专栏', '活动', '社区中心', '直播', '课堂', '新歌热榜']
</script>

<template>
  <div
    class="flex fixed min-w-[1100px] max-w-[2560px] w-full justify-center bg-white z-[1001] top-[63px] shadow-channel tracking-[2px] h-14 border-b-[1px] py-[14px] group hover:h-[150px]"
  >
    <div class="flex wrap justify-between overflow-hidden w-full">
      <!-- 左 -->
      <div class="flex text-[14px] text-gray-600 w-60 space-x-6">
      ...
      <!-- 分割线 -->
      <div class="h-[14px] w-[1px] mr-[10px] bg-slate-400 self-center"></div>
      <!-- 右 -->
      <div class="relative inline-grid flex-1 h-full grid-cols-15 space-x-[10px]">
        <div class="inline-grid col-span-12 grid-cols-12 grid-rows-3 gap-y-[14px] gap-x-[10px]">
          <a
            v-for="(item, i) in home.channelCategory"
            :key="i"
            href="#"
            class="box-content inline-block w-full border-solid border-gray-400 rounded-md bg-slate-200 text-center font-normal text-[14px] leading-[26px] h-[26px] col-span-1"
          >
            {{ item }}
          </a>
        </div>
        <div class="inline-grid col-span-3 grid-cols-3 grid-rows-3 gap-y-[14px] gap-x-[10px]">
          <a
            v-for="(item, i) in arr"
            :key="i"
            href="#"
        ...
        <svg
          ...
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          class="absolute left-full translate-x-2 translate-y-2 transition duration-300 inline-block ml-1 group-hover:rotate-180"
        >
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
